...
// Chart period button codes → readable labels for the "Weekly ▾" dropdown
...
// ─── Reusable chart period pill buttons ──────────────────────────────────────
...
const TrendPill = ({ trend, change, light }) => (
  <Stack direction="row" alignItems="center" spacing={0.3}
    sx={{ px: 0.8, py: 0.2, borderRadius: 10, display: "inline-flex",
...
// ScrollChart is defined OUTSIDE the main component to avoid remounting on every render
...
export default function Dashboard() {
  ...
  // ── Global period (overview cards) ───────────────────────────────────────
  const [period, setPeriod]           = useState("month");
  const [customStart, setCustomStart] = useState(null);
  const [customEnd, setCustomEnd]     = useState(null);

  // ── Independent chart periods ─────────────────────────────────────────────
  const [spPeriod, setSpPeriod]       = useState("1M"); // Sales & Purchase chart
  const [statsPeriod, setStatsPeriod] = useState("1M"); // Sales Stats (stacked) chart

  // ── Recent Transactions tab + period ─────────────────────────────────────
  const [txTab, setTxTab]             = useState("sale");
  const [txPeriod, setTxPeriod]       = useState("1M");

  // ── Bottom section shared "Weekly ▾" period ───────────────────────────────
  const [bottomPeriod, setBottomPeriod] = useState("1W");

  // ── Global dashboard query (overview cards + static lists) ───────────────
  const getQueryVars = () => {
    if (period === "custom")
  ...
  // ── Bottom section (Order Stats + Top Categories) ─────────────────────────
  ...
  // Chart-specific data (from independent queries)
  ...
  const makeBarOpts = (chartData, stacked = false) => {
    ...
    return {
      ...
      xaxis: { categories: chartData.labels, axisBorder: { show: false }, axisTicks: { show: false }, labels: { style: { colors: T.slate, fontSize: "11px" }, rotate: count > 20 ? -45 : 0, rotateAlways: count > 20 } },
  ...
  const printData = useMemo(() => {
    ...
    return { companyName, phone, email, address, invoiceNumber, invoiceDate: formatDateLong(new Date()), periodText, summaryRows, chartRows, infoRows, custRows, topProdRows, lowStockRows, rSalesRows, rTxRows, topCustRows, topCatRows, orderRows, catStatRows,
      hasChart: chartRows.length > 0, hasTopProd: topProdRows.length > 0, hasLowStock: lowStockRows.length > 0, hasRSales: rSalesRows.length > 0, hasRTx: rTxRows.length > 0, hasTopCust: topCustRows.length > 0, hasTopCat: topCatRows.length > 0 };
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterDayjs}>
      ...
      <Box sx={{ width: "100%", minHeight: "100vh", px: { xs: 1.5, sm: 2 }, pb: 5 }}>
        <Box sx={{ "@media print": { display: "none" } }}>

          {/* ── Toolbar ──────────────────────────────────────────────────── */}
          <Stack direction="row" justifyContent="space-between" alignItems="center" pt={2.5} pb={2}>
            <Typography sx={{ fontWeight: 800, fontSize: "1.1rem", color: "#1A2332" }}>{t("dashboard") || "Dashboard"}</Typography>
            <Stack direction="row" spacing={1.2}>
              <Tooltip title="Print">
                <Button onClick={handlePrint} variant="outlined" startIcon={<Print sx={{ fontSize: 15 }} />}
                  sx={{ borderColor: T.orange, color: T.orange, borderRadius: 2, textTransform: "none", fontWeight: 600, fontSize: "0.8rem", height: 34, "&:hover": { bgcolor: T.orangePale, borderColor: T.orange } }}>
                  {t("print") || "Print"}
                </Button>
              </Tooltip>
              <Button variant="contained" startIcon={<Download sx={{ fontSize: 15 }} />} onClick={handleExportExcel}
                sx={{ bgcolor: T.orange, borderRadius: 2, textTransform: "none", fontWeight: 600, fontSize: "0.8rem", height: 34, boxShadow: "0 2px 8px rgba(249,115,22,.3)", "&:hover": { bgcolor: "#EA6C10" } }}>
          ...
          <Grid container spacing={1.5} sx={{ mb: 1.5 }}>
            {[
              ...
              { title: t("total_sales_return")    || "Total Sales Return",    value: overview.totalSalesReturn?.value     || 0, change: overview.totalSalesReturn?.percentageChange     || 0, trend: overview.totalSalesReturn?.trend     || "down", bg: T.navyMid },
            ...
            <Grid size={{ xs: 12, md: 8 }}>
              <Card sx={cardSx}>
                <CardContent>
                  ...
                  {spLoading
                    ? <LinearProgress sx={{ borderRadius: 4, bgcolor: T.orangePale, "& .MuiLinearProgress-bar": { bgcolor: T.orange }, mb: 1 }} />
                    : null}
                  <ScrollChart count={spChart.labels.length}>
                    <Chart
                      options={makeBarOpts(spChart, false)}
                      series={[
                        { name: t("sales")     || "Total Sales",    data: spChart.sales     },
                        { name: t("purchases") || "Total Purchase", data: spChart.purchases },
                      ]}
                      type="bar" height={280}
                    />
                  </ScrollChart>
              ...
              <Card sx={{ ...cardSx, height: "100%" }}>
                <CardContent>
                  ...
                  <Stack direction="row" alignItems="center" spacing={1.5}>
                    <Box sx={{ flexShrink: 0 }}>
                      <Chart options={customerDonutOpts} series={[custOverview.firstTime || 0, custOverview.return || 0]} type="donut" height={130} width={130} />
                    </Box>
                          ...
                          <TableRow key={idx} hover sx={{ "&:last-child td": { border: 0 }, "&:hover": { bgcolor: T.orangePale } }}>
                            ...
                            <TableCell align="right">
                              <Chip
                                ...
                                size="small"
                                sx={{ fontWeight: 700, fontSize: "0.65rem", borderRadius: 6, height: 20,
            ...
            <Grid size={{ xs: 12, md: 7 }}>
              <Card sx={cardSx}>
                <CardContent>
                  ...
                  {/* ✅ Independent period pills for stacked chart */}
                  ...
                  {statsLoading
                    ? <LinearProgress sx={{ borderRadius: 4, bgcolor: T.orangePale, "& .MuiLinearProgress-bar": { bgcolor: T.orange }, mb: 1 }} />
                    : null}
                  <ScrollChart count={statsChart.labels.length}>
                    <Chart
                      options={makeBarOpts(statsChart, true)}
                      series={[
                        { name: t("sales")     || "Sales",    data: statsChart.sales     },
                        { name: t("purchases") || "Purchase", data: statsChart.purchases },
                      ]}
                      type="bar" height={250}
                    />
                  </ScrollChart>
              ...
              <Card sx={{ ...cardSx, height: "100%" }}>
                <CardContent>
                  <Stack direction="row" justifyContent="space-between" alignItems="center" mb={0}>
                    ...
                    {/* ✅ Period pills for transaction table */}
                    <PeriodPills active={txPeriod} onChange={setTxPeriod} />
                  </Stack>

                  {/* ✅ Wired tabs — each calls its own backend query */}
                  ...
                  <TableContainer>
                    <Table size="small">
                      <TableHead><TableRow>
                        ...
                        <TableCell align="right" sx={thSx}>Amount</TableCell>
                      </TableRow></TableHead>
                      <TableBody>
                        {recentTx.length === 0 && !txLoading ? (
                          <TableRow>
                            <TableCell colSpan={4} align="center" sx={{ py: 3, color: T.slate, fontSize: "0.78rem" }}>
                              No records found
                            </TableCell>
                          </TableRow>
                          ...
                          return (
                            <TableRow key={idx} hover sx={{ "&:last-child td": { border: 0 }, "&:hover": { bgcolor: T.orangePale } }}>
                              ...
                              <TableCell><Chip label={item.status} size="small" sx={{ fontWeight: 700, fontSize: "0.62rem", borderRadius: 6, height: 18, bgcolor: sc.bg, color: sc.color }} /></TableCell>
          ...
          <Grid container spacing={1.5}>
            ...
            <Grid size={{ xs: 12, md: 4 }}>
              <Card sx={{ ...cardSx, height: "100%" }}>
                <CardContent>
                  {/* ✅ "Weekly ▾" dropdown wired — shared with Order Statistics */}
                  <SectionTitle action={
                    <PeriodDropdown active={bottomPeriod} onChange={setBottomPeriod} />
                  }>{t("top_categories") || "Top Categories"}</SectionTitle>
                  {bottomLoading
                    ? <LinearProgress sx={{ borderRadius: 4, bgcolor: T.orangePale, "& .MuiLinearProgress-bar": { bgcolor: T.orange }, mb: 1 }} />
                    : null}
                  <Stack direction="row" alignItems="center" spacing={1.5}>
                    <Box sx={{ flexShrink: 0 }}>
                      <Chart options={categoryDonutOpts} series={topCategories.map(c => c.salesAmount)} type="donut" height={160} width={160} />
                    </Box>
                    <Stack spacing={1.2} flex={1}>
                  ...
                  <Stack direction="row" justifyContent="space-around">
                    {[{ label: t("total_categories") || "Categories", value: catStats.totalCategories }, { label: t("total_products") || "Products", value: catStats.totalProducts }].map((s, i) => (
            ...
            <Grid size={{ xs: 12, md: 5 }}>
              <Card sx={{ ...cardSx, height: "100%" }}>
                <CardContent>
                  {/* ✅ Same bottomPeriod state — both cards update together */}
                  <SectionTitle action={
                    <PeriodDropdown active={bottomPeriod} onChange={setBottomPeriod} />
                  }>{t("order_statistics") || "Order Statistics"}</SectionTitle>
                  {bottomLoading
                    ? <LinearProgress sx={{ borderRadius: 4, bgcolor: T.orangePale, "& .MuiLinearProgress-bar": { bgcolor: T.orange }, mb: 1 }} />
                    : null}
                  <Chart options={orderStatsOpts} series={[{ name: t("orders") || "Orders", data: orderStats.values }]} type="bar" height={260} />
                </CardContent>
              </Card>
            </Grid>
          </Grid>
